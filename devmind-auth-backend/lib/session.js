...
export function verifyState(state) {
  ...
  if (!payload.createdAt || Date.now() - payload.createdAt > DEFAULT_TTL_SECONDS * 1000) {
...
export async function storeOneTimeCode(record, ttlSeconds = DEFAULT_TTL_SECONDS) {
  ...
  if (kv) {
    await kv.set(`devmind:oauth:${code}`, value, { ex: ttlSeconds })
  } else {
    memoryStore.set(code, value)
    setTimeout(() => memoryStore.delete(code), ttlSeconds * 1000).unref?.()
  }
  return code
}
...
export function errorRedirect(res, provider, error) {
  ...
  const params = new URLSearchParams({
    provider,
    error: error?.message || String(error || 'OAuth failed'),
  })
