import { json, openExchangeRecord } from '../../lib/session.js'

function normalizeProfile(provider, profile) {
  if (provider === 'google') return { id: profile.sub, name: profile.name, email: profile.email, avatarUrl: profile.picture, provider }
  if (provider === 'github') return { id: profile.id, login: profile.login, name: profile.name, email: profile.email, avatarUrl: profile.avatar_url, provider }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const record = openExchangeRecord(body.code)
    return json(res, 200, {
      ok: true,
