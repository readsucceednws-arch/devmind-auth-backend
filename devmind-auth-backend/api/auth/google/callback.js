...
async function fetchJson(url, options) {
  ...
  if (!response.ok || payload.error) {
    throw new Error(payload.error_description || payload.error || `Request failed: ${response.status}`)
  }
  return payload
}
...
export default async function handler(req, res) {
  try {
    ...
    const exchangeCode = await storeOneTimeCode({ provider: 'google', token, profile })
